...
const GameUserReviewUnit = ({review}) => <div>
    <span className="avatar ball"><Icon type="person" size="16" /></span>
    <span className={`score ball score${_getRounded(review.score)}`}>{t('score')}: <strong>{review.score}</strong></span>
    <div className="user">{t('by')}: <strong>{review.userInfo && _getName(review.userInfo.name)}</strong></div>
    <time dateTime={review.added}>{new Date(review.added).toLocaleDateString(t('date-locale'))}</time>
