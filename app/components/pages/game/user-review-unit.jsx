...
import { ANONYMOUS_REVIEWER } from '../../../constants'
...
const _getName = ({ userInfo, nickname }) => {
	if (userInfo && userInfo.name) return name
	if (nickname && nickname === ANONYMOUS_REVIEWER) return t('anonymous-review')
	return nickname || t('unknown')
}
...
const GameUserReviewUnit = ({ review }) => <div>
	...
	<div className="user">
		{t('by')}: <strong>{_getName(review)}</strong>
	</div>
